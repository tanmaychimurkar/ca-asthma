...
)  # todo: joint intent classification and slot filling classification for getting time, only for report
# atist dataset?
candidate_label_current_time = [
    "current time",
    "general question",
]  # todo: if general, then only gpt3 output, else gpt3/weather/heartrate

user_mock_data = pd.read_csv(
    current_path + "/../../model_checkpoints/mock_healthcare.csv"
)
...
def current_response():
    ...
    if AQI < 50:
        ...
        weather_response = (
            f"Air quality Index is moderate, decide what to do Juli placeholder"
        )
    elif 101 < AQI < 150:
        weather_response = (
            f"Air quality is Unhealthy for sensitive group. Placeholder for Juli"
        )
    elif AQI > 150:
